...
- **Auto-fire it (optional, the 0-token version)** — a `Stop` hook that, when the working tree is dirty (= work happened) and this batch hasn't been triaged yet (git-status fingerprint marker), injects the triage reminder once. Three safety gates are mandatory: honor `stop_hook_active` (block at most once, never loop), stay silent on a clean tree (don't nag read-only sessions), and de-dupe by fingerprint. Any error → exit 0 (never obstruct stop).
